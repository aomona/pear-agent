...
- Follow-up: the Core integration contract now imports the shipped outing Goal, Plan, and initial WorldState fixtures directly, so Repository → Event → Snapshot behavior exercises the example data.
- Follow-up: Runtime Events in the integration contract are constructed through `runtimeEventSchema.parse()` without a type assertion.
- Removed Core's narrow `rootDir` setting so tsgo can validate the integration test's workspace fixture import without introducing a cyclic package dependency.
